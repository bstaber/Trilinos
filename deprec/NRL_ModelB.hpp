...
public:
    ...
    Interface_dirichlet(Epetra_Comm & comm, Teuchos::ParameterList & Parameters){
        ...
        mu1 = Teuchos::getParameter<double>(Parameters.sublist("Model_B"), "mu1");
        mu2 = Teuchos::getParameter<double>(Parameters.sublist("Model_B"), "mu2");
        mu4 = Teuchos::getParameter<double>(Parameters.sublist("Model_B"), "mu4");
        beta4 = Teuchos::getParameter<double>(Parameters.sublist("Model_B"), "beta4");
        beta5 = Teuchos::getParameter<double>(Parameters.sublist("Model_B"), "beta5");
        epsilon1 = Teuchos::getParameter<double>(Parameters.sublist("Model_B"), "epsilon1");
        
        mu5 = mu4*beta4/beta5;
        epsilon2 = 2.0*mu1 + 4.0*mu2 + 2.0*epsilon1 + 2.0*mu5*beta5;
        
        plyagl = Teuchos::getParameter<double>(Parameters.sublist("Model_B"), "angle");
    ...
    void apply_dirichlet_conditions(Epetra_FECrsMatrix & K, Epetra_FEVector & F, double & displacement){
        ...
        //if (n_bc_dof>0){
            ...
            error=K.Apply(v,rhs_dir);
            error=F.Update(-1.0,rhs_dir,1.0);
    ...
    void get_constitutive_tensors(Epetra_SerialDenseMatrix & deformation_gradient, Epetra_SerialDenseVector & piola_stress, Epetra_SerialDenseMatrix & tangent_piola){
        ...
        for (unsigned int i=0; i<6; ++i){
            ...
            piola_stress(i) = 2.0*mu1*eye(i) + 2.0*mu2*(I1*eye(i) - c(i)) + det*pressure*L(i) + 2.0*mu4*beta4*pI4*M(i) + 2.0*mu5*beta5*pJ5*dJ5(i);
        }

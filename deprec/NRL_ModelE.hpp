...
public:
    ...
    Interface_dirichlet(Epetra_Comm & comm, Teuchos::ParameterList & Parameters){
        ...
        m1 = Teuchos::getParameter<double>(Parameters.sublist("Model"), "m1");
        m2 = Teuchos::getParameter<double>(Parameters.sublist("Model"), "m2");
        ...
        m = 2.0*m1 + m2;
        mu5 = mu4*beta4*std::pow(m,beta4-beta5)/beta5;
        epsilon2 = 2.0*epsilon1 + 2.0*mu5*beta5*std::pow(m,beta5);
    ...
    void get_constitutive_tensors(Epetra_SerialDenseMatrix & deformation_gradient, Epetra_SerialDenseVector & piola_stress, Epetra_SerialDenseMatrix & tangent_piola){
        ...
        M(0) = m1*sin_plyagl*sin_plyagl+m2*cos_plyagl*cos_plyagl; M(1) = m1*cos_plyagl*cos_plyagl+m2*sin_plyagl*sin_plyagl; M(2) = m1; M(3) = 0.0; M(4) = 0.0; M(5) = -m1*cos_plyagl*sin_plyagl + m2*cos_plyagl*sin_plyagl;
        ...
        for (unsigned int i=0; i<6; ++i){
            ...
            piola_stress(i) = det*pressure*L(i) + 2.0*mu4*beta4*pI4*M(i) + 2.0*mu5*beta5*pJ5*dJ5(i);
        }

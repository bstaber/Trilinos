...
public:
    ...
    /*void get_constitutive_tensors_static_condensation(Epetra_SerialDenseMatrix & deformation_gradient, double & det, Epetra_SerialDenseVector & piola_isc, Epetra_SerialDenseVector & piola_vol, Epetra_SerialDenseMatrix & tangent_piola_isc, Epetra_SerialDenseMatrix & tangent_piola_vol){
        ...
        double alpha, b1, b2, d1, d2;
        double I1, I4_1, I4_2, I5_1, I5_2, K3_1, K3_2;
        ...
        Epetra_SerialDenseVector CC(6);
        Epetra_SerialDenseVector CMMC1(6);
        Epetra_SerialDenseVector CMMC2(6);
        Epetra_SerialDenseVector D(6);
        Epetra_SerialDenseVector L(6);
        Epetra_SerialDenseVector piola_nh(6);
        Epetra_SerialDenseVector piola_ani(6);
        Epetra_SerialDenseVector dK3_1(6);
        Epetra_SerialDenseVector dK3_2(6);
        ...
        I1 = C(0) + C(1) + C(2);
        I4_1 = C(0)*M1(0) + C(1)*M1(1) + C(2)*M1(2) + 2.0*C(5)*M1(5) + 2.0*C(4)*M1(4) + 2.0*C(3)*M1(3);
        I4_2 = C(0)*M2(0) + C(1)*M2(1) + C(2)*M2(2) + 2.0*C(5)*M2(5) + 2.0*C(4)*M2(4) + 2.0*C(3)*M2(3);
        I5_1 = CC(0)*M1(0) + CC(1)*M1(1) + CC(2)*M1(2) + 2.0*CC(5)*M1(5) + 2.0*CC(4)*M1(4) + 2.0*CC(3)*M1(3);
        I5_2 = CC(0)*M2(0) + CC(1)*M2(1) + CC(2)*M2(2) + 2.0*CC(5)*M2(5) + 2.0*CC(4)*M2(4) + 2.0*CC(3)*M2(3);
        K3_1 = I1*I4_1-I5_1;
        K3_2 = I1*I4_2-I5_2;
        ...
        d1 = 0.0;
        if (K3_1>2.0){
            temp = std::pow(K3_1-2.0,beta4-1.0);
            b1 = 2.0*mu4*beta4*temp;
            d1 = 4.0*mu4*beta4*(beta4-1.0)*temp/(K3_1-2.0);
        }
        b2 = 0.0;
        d2 = 0.0;
        if (K3_2>2.0){
            temp = std::pow(K3_2-2.0,beta4-1.0);
            b2 = 2.0*mu4*beta4*temp;
            d2 = 4.0*mu4*beta4*(beta4-1.0)*temp/(K3_2-2.0);
        }
        
        for (unsigned int i=0; i<6; ++i){
            dK3_1(i) = I4_1*eye(i) + I1*M1(i) - CMMC1(i);
            dK3_2(i) = I4_2*eye(i) + I1*M2(i) - CMMC2(i);
            piola_nh(i) = alpha*mu1*(eye(i) - (1.0/3.0)*I1*L(i));
            piola_ani(i) = b1*dK3_1(i) + b2*dK3_2(i);
            piola_vol(i) = det*L(i);
        }
        ...
        sym_tensor_product((2.0/3.0)*alpha*mu1*I1,L,L,tangent_piola_isc,1.0);
        tensor_product(-(2.0/9.0)*alpha*mu1*I1,L,L,tangent_piola_isc,1.0);
        
        tensor_product(d1,dK3_1,dK3_1,tangent_piola_isc,1.0);
        tensor_product(d2,dK3_2,dK3_2,tangent_piola_isc,1.0);
        
        tensor_product(2.0*b1,eye,M1,tangent_piola_isc,1.0);
        tensor_product(2.0*b1,M1,eye,tangent_piola_isc,1.0);
        
        sym_tensor_product(-2.0*b1,M1,eye,tangent_piola_isc,1.0);
        sym_tensor_product(-2.0*b1,eye,M1,tangent_piola_isc,1.0);
        
        tensor_product(2.0*b2,eye,M2,tangent_piola_isc,1.0);
        tensor_product(2.0*b2,M2,eye,tangent_piola_isc,1.0);
        
        sym_tensor_product(-2.0*b2,eye,M2,tangent_piola_isc,1.0);
        sym_tensor_product(-2.0*b2,M2,eye,tangent_piola_isc,1.0);

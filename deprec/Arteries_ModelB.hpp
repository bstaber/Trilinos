...
public:
    ...
    void get_constitutive_tensors_static_condensation(Epetra_SerialDenseMatrix & deformation_gradient, double & det, Epetra_SerialDenseVector & piola_isc, Epetra_SerialDenseVector & piola_vol, Epetra_SerialDenseMatrix & tangent_piola_isc, Epetra_SerialDenseMatrix & tangent_piola_vol){
        ...
        double T4_1 = 0.0;
        if (I4_1>1.0){
            S4_1 = std::pow(I4_1-1.0,beta4-1.0);
            T4_1 = S4_1/(I4_1-1.0);
        }
        double S4_2 = 0.0;
        double T4_2 = 0.0;
        if (I4_2>1.0){
            S4_2 = std::pow(I4_2-1.0,beta4-1.0);
            T4_2 = S4_2/(I4_2-1.0);
        }
        ...
        for (unsigned int i=0; i<6; ++i){
            ...
            piola_isc(i) = piola_nh(i) + ( (3.0*mu2*pI2*(I1*eye(i)-C(i)))/(det*det) ) - ( (2.0*mu2*pI2*I2*L(i))/(det*det) ) + 2.0*mu4*beta4*( S4_1*M1(i) + S4_2*M2(i) );
            piola_vol(i) = det*L(i);
            D(i) = I1*eye(i) - C(i);
        }
        ...
        scalarAB = (-2.0/9.0)*mu1*alpha*I1 + 4.0*mu2*pI2*I2/(det*det);
        tensor_product(scalarAB,L,L,tangent_piola_isc,1.0);
        
        scalarAB = (2.0/3.0)*mu1*alpha*I1 + 4.0*mu2*pI2*I2/(det*det);
        ...
        scalarAB = 4.0*mu4*beta4*(beta4-1.0)*T4_1;
        tensor_product(scalarAB,M1,M1,tangent_piola_isc,1.0);
        
        scalarAB = 4.0*mu4*beta4*(beta4-1.0)*T4_2;
